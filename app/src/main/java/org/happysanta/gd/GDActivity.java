...
public class GDActivity extends Activity implements Runnable {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (true) {
			...
			titleLayout.setBackgroundColor(0x10000000);
			...
			menuTitleTextView.setTextColor(0xffffffff);
			...
			for (int i = 0; i < 3; i++) {
				...
				row.setBackgroundColor(0x10ffffff);
				for (int j = 0; j < 3; j++) {
					LinearLayout btn = new LinearLayout(this);
					TextView btnText = new TextView(this);
					btnText.setText(String.valueOf(i * 3 + j + 1));
					btnText.setTextColor(0xff00ffff);
					btnText.setTextSize(30);
